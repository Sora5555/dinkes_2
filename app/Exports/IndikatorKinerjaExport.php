...
class IndikatorKinerjaExport implements FromCollection, WithHeadings, WithEvents, WithCustomStartCell
{
    /**
    ...
    */
    ...
    public function collection()
    {
        ...
        $mappedData = $AngkaKematian->map(function($item, $no) {
            return [
                'td0' => $no++,
                'td1' => $item->nama_rumah_sakit,
                'td2' => $item->jumlah_tempat_tidur,
                'td3' => $item->pasien_keluar_hidup_mati_L + $item->pasien_keluar_hidup_mati_P,
                'td4' => $item->jumlah_hari_perawatan,
                'td5' => $item->jumlah_lama_dirawat,
                'td6' => ($item->jumlah_tempat_tidur>0?number_format($item->jumlah_hari_perawatan/($item->jumlah_tempat_tidur * 365) * 100, 2):0),
                'td7' => ($item->jumlah_tempat_tidur>0?number_format(($item->pasien_keluar_hidup_mati_L + $item->pasien_keluar_hidup_mati_P)/($item->jumlah_tempat_tidur), 2):0),
                'td8' => ($item->pasien_keluar_hidup_mati_L + $item->pasien_keluar_hidup_mati_P>0?number_format((($item->jumlah_tempat_tidur * 365)-$item->jumlah_hari_perawatan)/($item->pasien_keluar_hidup_mati_L + $item->pasien_keluar_hidup_mati_P), 2):0),
                'td9' => ($item->pasien_keluar_hidup_mati_L + $item->pasien_keluar_hidup_mati_P>0?number_format(($item->jumlah_lama_dirawat)/($item->pasien_keluar_hidup_mati_L + $item->pasien_keluar_hidup_mati_P), 2):0),
            ];
        })->toArray();
...
public function registerEvents(): array
{
    return [
        AfterSheet::class => function(AfterSheet $event) {
            ...
            $sheet->getStyle('A5:J4')->applyFromArray([
               'borders' => [
                    'bottom' => [
                        'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN,
                        'color' => ['argb' => '000000'], // Black color
