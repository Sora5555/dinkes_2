...
class KelompokUmurExport implements FromCollection, WithHeadings, WithEvents, WithCustomStartCell
{
    /**
    ...
    */
    ...
    public function collection()
    {
        ...
        // });
        // $no = 0;
        $mappedData = $kelompokUmur->map(function($unit, $no) {
            return [
                'no' => $no++,
                'kelompok_umur' => $unit->batas_bawah." - ".$unit->batas_atas,
                'laki_laki' => $unit->laki_laki,
                'perempuan' => $unit->perempuan,
                'laki_laki_perempuan' => $unit->laki_laki + $unit->perempuan,
                'rasio_jenis_kelamin' => $unit->perempuan>0?number_format($unit->laki_laki/$unit->perempuan * 100, 2):0,
            ];
        })->toArray();
        ...
        $mappedData[] = [
            ...
            'total_laki_laki_perempuan' =>  $total_laki_laki + $total_perempuan,
            'total_rasio_jenis_kelamin' => $total_perempuan > 0?number_format($total_laki_laki/$total_perempuan * 100, 2):0,
        ];
...
public function registerEvents(): array
{
    return [
        AfterSheet::class => function(AfterSheet $event) {
            ...
            $sheet->getStyle('A5:F4')->applyFromArray([
               'borders' => [
                    'bottom' => [
                        'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN,
                        'color' => ['argb' => '000000'], // Black color
