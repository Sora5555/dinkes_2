...
class FasilitasKesehatanExport implements FromCollection, WithHeadings, WithEvents, WithCustomStartCell
{
    /**
    ...
    */
    ...
    public function collection()
    {
        ...
        $mappedData = $RumahSakit->map(function($item, $no) {
            return [
                ...
                'td3' => $item->swasta,
                'td2' => $item->ormas,
                'td3' => ($item->kemenkes + $item->pemprov + $item->pemkot + $item->tni_polri + $item->bumn + $item->swasta + $item->ormas),
            ];
        })->toArray();

        $mappedData[] = $Puskesmas->map(function($item, $no) {
            return [
                'td0' => $no++,
                'td1' => $item->fasilitas_kesehatan,
                'td2' => $item->kemenkes,
        ...
        $mappedData[] = $Farmasi->map(function($item, $no) {
            return [
                'td0' => $no++,
                'td1' => $item->fasilitas_kesehatan,
                'td2' => $item->kemenkes,
    ...
    public function headings(): array
    {
        return [
            ...
            ['No', 'Fasilitas Kesehatan', 'Kemenkes','Pem.prov', 'Pem.kab/kota', 'TNI/POLRI', 'BUMN', 'Swasta', 'Organisasi Kemasyarakatan', 'Jumlah'],
