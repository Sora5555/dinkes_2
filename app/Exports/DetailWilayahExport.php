...
class DetailWilayahExport implements FromCollection, WithHeadings, WithEvents, WithCustomStartCell
{
    /**
    ...
    public function collection()
    {
        ...
        $mappedData = $unit_kerja->map(function($unit, $no) {
            return [
                'no' => $no++,
                'desa_name' => $unit->nama,
                'luas_wilayah' => $unit->luas_wilayah,
                'desa' => $unit->Desa()->count(),
                'Kelurahan' => $unit->kelurahan,
                'Desa_Kelurahan' => $unit->Desa()->count() + $unit->kelurahan,
                'jumlah_penduduk' => $unit->jumlah_penduduk,
                'jumlah_rumah_tangga' => $unit->jumlah_rumah_tangga,
                'rata_rata_jiwa_rumah_tangga' => ($unit->jumlah_rumah_tangga > 0 ? number_format(($unit->jumlah_penduduk/($unit->jumlah_rumah_tangga))*100, '2'):0)."%",
                'kepadatan_penduduk_per_km2' => ($unit->luas_wilayah > 0 ? number_format(($unit->jumlah_penduduk/($unit->luas_wilayah))*100, '2'):0). "%",
            ];
        })->toArray();
