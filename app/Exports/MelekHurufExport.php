...
class MelekHurufExport implements FromCollection, WithHeadings, WithEvents, WithCustomStartCell
{
    /**
    ...
    */
    ...
    public function collection()
    {
        ...
        $mappedData[] = [
            'td0' => 1,
            'td1' => 'PENDUDUK BERUMUR 15 TAHUN KE ATAS',
            'td2' => $laki_laki,
            'td3' => $perempuan,
            'td4' =>  $laki_laki + $perempuan,
            'td5' => '',
            'td6' => '',
            'td7' => '',
        ];
        ...
        $mappedData = $MelekHuruf->map(function($item, $no) {
            ...
            return [
                ...
                'td2' => $item->laki_laki,
                'td3' => $item->perempuan,
                'td4' => $laki_laki>0?number_format($item->laki_laki/$laki_laki * 100, 2):0,
                'td5' => $perempuan>0?number_format($item->perempuan/$perempuan * 100, 2):0,
                'td6' => $perempuan + $laki_laki>0?number_format(($item->perempuan + $item->laki_laki)/($perempuan + $laki_laki) * 100, 2):0,
            ];
        })->toArray();
...
public function registerEvents(): array
{
    return [
        AfterSheet::class => function(AfterSheet $event) {
            ...
            $sheet->mergeCells('A1:H1'); // Shifted from 'A1:W1' to 'A3:W3' (two extra rows)
            $sheet->mergeCells('A2:H2'); // Shifted from 'A1:W1' to 'A3:W3' (two extra rows)
            $sheet->mergeCells('A3:H3'); // Shifted from 'A1:W1' to 'A3:W3' (two extra rows)
