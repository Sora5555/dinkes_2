...
class Table86Controller extends Controller
{
    protected $routeName = 'table_86';
    protected $viewName = 'input_data.table_86';
    protected $title = 'CAKUPAN VAKSINASI COVID-19 DOSIS 1 MENURUT KECAMATAN DAN PUSKESMAS';
    ...
    public function add(Request $request)
    {
        ...
        $fillable = (new Table86())->getFillable(); // Get only fillable attributes
        $sessionYear = Session::get('year'); // Retrieve the year from the session

    
        foreach (Auth::user()->unit_kerja->Desa()->get() as $desa) {
            if ($desa->filterTable86($sessionYear, $desa->id)) {
                continue;
            } else {
                $data = [];
    ...
    {
        try {
            $data = Table86::where('id', $request->id)->first();
            ...
            $Grandsasaran_6_11 = Table86::whereHas('Desa', function ($a) {
                    $a->where('unit_kerja_id', Auth::user()->unit_kerja_id);
                })->sum('sasaran_6_11');
            $Grandhasil_vaksinasi_6_11 = Table86::whereHas('Desa', function ($a) {
                $a->where('unit_kerja_id', Auth::user()->unit_kerja_id);
            })->sum('hasil_vaksinasi_6_11');
            $Grandpersen_6_11 = number_format(($Grandhasil_vaksinasi_6_11 / $Grandsasaran_6_11) * 100, 2) . '%';

            $Grandsasaran_12_17 = Table86::whereHas('Desa', function ($a) {
                $a->where('unit_kerja_id', Auth::user()->unit_kerja_id);
            })->sum('sasaran_12_17');
            $Grandhasil_vaksinasi_12_17 = Table86::whereHas('Desa', function ($a) {
                $a->where('unit_kerja_id', Auth::user()->unit_kerja_id);
            })->sum('hasil_vaksinasi_12_17');
            $Grandpersen_12_17 = number_format(($Grandhasil_vaksinasi_12_17 / $Grandsasaran_12_17) * 100, 2) . '%';

            $Grandsasaran_18_59 = Table86::whereHas('Desa', function ($a) {
                $a->where('unit_kerja_id', Auth::user()->unit_kerja_id);
            })->sum('sasaran_18_59');
            $Grandhasil_vaksinasi_18_59 = Table86::whereHas('Desa', function ($a) {
                $a->where('unit_kerja_id', Auth::user()->unit_kerja_id);
            })->sum('hasil_vaksinasi_18_59');
            $Grandpersen_18_59 = number_format(($Grandhasil_vaksinasi_18_59 / $Grandsasaran_18_59) * 100, 2) . '%';

            $Grandsasaran_60_up = Table86::whereHas('Desa', function ($a) {
                $a->where('unit_kerja_id', Auth::user()->unit_kerja_id);
            })->sum('sasaran_60_up');
            $Grandhasil_vaksinasi_60_up = Table86::whereHas('Desa', function ($a) {
                $a->where('unit_kerja_id', Auth::user()->unit_kerja_id);
