                ...
                <div class="card-body">
                    <div class="row justify-content-start mb-2">
                        <div class="col-md-10 d-flex justify-content-start gap-3">
                            @if(Auth::user()->downloadFile('Kunjungan', Session::get('year')))
                            <form action="/upload/general" method="post" class="d-flex gap-5" enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="name" value="Kunjungan" id="">
                                <input type="file" name="file_upload" id="" {{Auth::user()->downloadFile('Kunjungan', Session::get('year'))->status == 1 ?"disabled":""}} class="form-control" placeholder="upload PDF file">
                                <button type="submit" class="btn btn-success" {{Auth::user()->downloadFile('Kunjungan', Session::get('year'))->status == 1?"disabled":""}}>Upload</button>
                            ...
                            @endif
                            @if(Auth::user()->hasFile('Kunjungan', Session::get('year')))
                                <a type="button" class="btn btn-warning" href="{{ Auth::user()->downloadFile('Kunjungan', Session::get('year'))->file_path.Auth::user()->downloadFile('Kunjungan', Session::get('year'))->file_name }}" download="" ><i class="mdi mdi-note"></i>Download pdf file</a>
                            @endif
                            <a type="button" class="btn btn-warning" href="{{ route('Kunjungan.excel') }}" ><i class="mdi mdi-note"></i>Report</a>
                            <a type="button" class="btn btn-primary" href="{{ route('Kunjungan.add', ['year' => Session::get('year')]) }}"><i class="mdi mdi-plus"></i>Add</a>
                        </div>
                    </div>
                        ...
                        <table id="data" class="table table-bordered dt-responsive" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                            ...
                            <tbody>
                                ...
                                <tr style={{$key % 2 == 0?"background: gray":""}}>
                                    ...
                                    <td><input type="checkbox" name="lock" {{$item->kunjungan_lock_get(Session::get('year')) == 1 ? "checked":""}} class="data-lock" id="{{$item->id}}"></td>
                                    @if(isset($item->user) && $item->user->downloadFile('Kunjungan', Session::get('year')))
                                    <td><input type="checkbox" name="lock_upload" {{$item->user->downloadFile('Kunjungan', Session::get('year'))->status == 1 ? "checked":""}} class="data-lock-upload" id="{{$item->user->id}}"></td>
                                    @elseif(isset($item->user) && !$item->user->downloadFile('Kunjungan', Session::get('year')))
                                    <td><input type="checkbox" name="lock_upload" class="data-lock-upload" id="{{$item->user->id}}"></td>
                                    @else
                                    <td>-</td>
                                    @endif
                                    @if(isset($item->user) && $item->user->hasFile('Kunjungan', Session::get('year')))
                                    <td>
                                        @if($item->user->downloadFile('Kunjungan', Session::get('year'))->file_name != "-")
                                        <a type="button" class="btn btn-warning" href="{{ $item->user->downloadFile('Kunjungan', Session::get('year'))->file_path.$item->user->downloadFile('Kunjungan', Session::get('year'))->file_name }}" download="" ><i class="mdi mdi-note"></i>Download pdf file</a>
                                        @else
                                        -
                                        @endif
                                    </td>
                                ...
                                <tr style='{{$key % 2 == 0?"background: #e9e9e9":""}}'>
                                    ...
                                    <td><input type="number" {{$item->filterKunjungan(Session::get('year'))->status == 1?"disabled":""}} name="jalan_L" id="{{$item->filterKunjungan(Session::get('year'))->id}}" value="{{$item->filterKunjungan(Session::get('year'))->jalan_L}}" class="form-control data-input" style="border: none"></td>
                                    <td><input type="number" {{$item->filterKunjungan(Session::get('year'))->status == 1?"disabled":""}} name="jalan_P" id="{{$item->filterKunjungan(Session::get('year'))->id}}" value="{{$item->filterKunjungan(Session::get('year'))->jalan_P}}" class="form-control data-input" style="border: none"></td>
                                    <td id="jalan{{$item->filterKunjungan(Session::get('year'))->id}}">{{$item->filterKunjungan(Session::get('year'))->jalan_P + $item->filterKunjungan(Session::get('year'))->jalan_L}}</td>
                                    
                                    <td><input type="number" {{$item->filterKunjungan(Session::get('year'))->status == 1?"disabled":""}} name="inap_L" id="{{$item->filterKunjungan(Session::get('year'))->id}}" value="{{$item->filterKunjungan(Session::get('year'))->inap_L}}" class="form-control data-input" style="border: none"></td>
                                    <td><input type="number" {{$item->filterKunjungan(Session::get('year'))->status == 1?"disabled":""}} name="inap_P" id="{{$item->filterKunjungan(Session::get('year'))->id}}" value="{{$item->filterKunjungan(Session::get('year'))->inap_P}}" class="form-control data-input" style="border: none"></td>
                                    <td id="inap{{$item->filterKunjungan(Session::get('year'))->id}}">{{$item->filterKunjungan(Session::get('year'))->inap_L + $item->filterKunjungan(Session::get('year'))->inap_P}}</td>
                                    
                                    <td><input type="number" {{$item->filterKunjungan(Session::get('year'))->status == 1?"disabled":""}} name="jiwa_L" id="{{$item->filterKunjungan(Session::get('year'))->id}}" value="{{$item->filterKunjungan(Session::get('year'))->jiwa_L}}" class="form-control data-input" style="border: none"></td>
                                    <td><input type="number" {{$item->filterKunjungan(Session::get('year'))->status == 1?"disabled":""}} name="jiwa_P" id="{{$item->filterKunjungan(Session::get('year'))->id}}" value="{{$item->filterKunjungan(Session::get('year'))->jiwa_P}}" class="form-control data-input" style="border: none"></td>
                                    <td id="jiwa{{$item->filterKunjungan(Session::get('year'))->id}}">{{$item->filterKunjungan(Session::get('year'))->jiwa_L + $item->filterKunjungan(Session::get('year'))->jiwa_P}}</td>
...
@push('scripts')
    <script>
        ...
        let id = $(this).attr('id');
        let jalan = $(this).parent().parent().find(`#jalan${id}`);
        let inap = $(this).parent().parent().find(`#inap${id}`);
        let jiwa = $(this).parent().parent().find(`#jiwa${id}`);
		...
		$.ajax({
			...
			'data'	: {'id': id, 'mainFilter': 'filterKunjungan'},
			success	: function(res){
                console.log(res);
                ...
                let newRow = `
                    <tr class="detail-row" style="background: #f9f9f9;">
                        ...
                        <td></td>
                                        ...
                                        <td>${item.jalan_L}</td>
                                        <td>${item.jalan_P}</td>
                                        <td>${item.jalan_L + item.jalan_P}</td>
                                        
                                        <td>${item.inap_L}</td>
                                        <td>${item.inap_P}</td>
                                        <td>${item.inap_L + item.inap_P}</td>
        ...
        $("#filter").click(function(){
            let year = $("#tahun").val();
            window.location.href = "/kunjungan?year="+year;
    ...
    $('#datatable').on('click', '.btn-tambah-sasaran', function(){
        ...
        $('#data').on('click', '.data-lock-upload', function(){
			...
			'data'	: {'id': id, 'status': 1, 'year': year, 'name': "Kunjungan"},
			success	: function(res){
				console.log(res);
			}
		});
			...
			'type' 	: 'POST',
			'url'	: '{{ route("general.lock_upload") }}',
			'data'	: {'id': id, 'status': 0, 'year': year, 'name': "Kunjungan"},
			success	: function(res){
				console.log(res);
			}
		});
