    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0">Tujuan Renstra</h4>
...
if(id != ""){
    $.ajax({
            ...
            success: (res) => {
                if(res.status == "success"){
                        ...
                        console.log(res.data);
                        merujuk_rpjmd(res.visi, res.misi, res.tujuan, res.sasaran, res.tujuanRenstra);
                        ...
                        misi.forEach(element => {
                            td += `<tr>
                                ...
                                <td><button class="btn col-md-4 btn-mod" renstra_id="${element.id}" style="background: #42d408"><i class="mdi mdi-plus"></i></button>   
                                    </td>
                                </tr>`
                            tujuan.forEach(elementTr => {
                                ...
                                <td>
                                    <form method="POST" action="/visi_renstra/${elementTr.id}">
                                            @csrf
                                            {{ method_field('DELETE') }}
                                            <div class="btn-group" role="group" aria-label="Action Button">
                                                    <input type="hidden" name="_method" value="DELETE">
                                                    <a type="button" class="btn btn-sm btn-warning m-2 btn-mod2" id="${elementTr.id}"><i class="fa fa-edit"></i></a>
    ...
    function merujuk_rpjmd(visi, misi, tujuan, sasaran, tujuanRenstra){
        let td = ''
        ...
        visi.forEach(element => {
            ...
            misi.forEach((elementMisi, index) => {
                ...
                <tbody>
                                <tr style="background:#c4dbf5" align="left">
                                    <th colspan="3"><span style="padding-left: 7px">Misi ${index + 1}: ${elementMisi.nama}</span></th>
                                    <th colspan="3"><button class="btn col-md-8 btn-mod" id="${elementMisi.id}" style="background: #42d408"><i class="mdi mdi-plus"></i></button></th>
                                </tr>
                `
                    ...
                    if(elementTr.misi_rpjmd_id == elementMisi.id && elementTr.induk_opd_id == valueOpd){
                            td += `
                                <tr style="background:#ffffff" align="left">
                                    ...
                                    <th>
                                        <form method="POST" action="/tujuan_renstra/${elementTr.id}">
                                            @csrf
                                            {{ method_field('DELETE') }}
                                            <div class="btn-group" role="group" aria-label="Action Button">
                                                    <input type="hidden" name="_method" value="DELETE">
    ...
    $('#table').on('click', '.btn-mod2', function(){
        ...
        $.ajax({
            type: "get",
            url: `{{url('apiEdit/tujuan_renstra')}}/${id}`,
            success: (res) => {
                if(res.status == "success"){
        ...
        <div class="mb-3 row">
            ...
            $('#EditForm').attr('action', `/tujuan_renstra/${id}`)
    ...
    $('.table').on('click', '.btn-mod', function(){
        let valueOpd = $('#induk_opd').val();
        let id = $(this).attr('id');
        let renstra_id = $(this).attr('renstra_id');

        if(valueOpd){
            let template = `
            {!! Form::open(['route'=>$route.'.store','method'=>'POST', 'id'=>'storeForm']) !!}
            <input name="misi_rpjmd_id" type="hidden" value="${id}">
            <input name="misi_renstra_id" type="hidden" value="${renstra_id}">
            <input name="induk_opd_id" type="hidden" value="${valueOpd}">
        <button type="button" id="tambah_misi" class="btn col-md-3" style="background: #42d408; font-weight: bold">Tambah Tujuan</button>
        <div id="misi_field">
        <div class="mb-3 row">
