    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0">Pelayanan Produktif</h4>

                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="javascript: void(0);">Input Data</a></li>
                        <li class="breadcrumb-item active">Pelayanan Produktif</li>
                    </ol>
                </div>
                    ...
                    <div class="row justify-content-start mb-2">
                        <div class="col-md-10 d-flex justify-content-start gap-3">
                            ...
                            <form action="/upload/general" method="post" class="d-flex gap-5" enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="name" value="PelayananProduktif" id="">
                                <input type="file" name="file_upload" id="" {{Auth::user()->downloadFile('PelayananProduktif', Session::get('year'))->status == 1 ?"disabled":""}} class="form-control" placeholder="upload PDF file">
                                <button type="submit" class="btn btn-success" {{Auth::user()->downloadFile('PelayananProduktif', Session::get('year'))->status == 1?"disabled":""}}>Upload</button>
                            ...
                            @endif
                            @if(Auth::user()->hasFile('PelayananProduktif', Session::get('year')))
                                <a type="button" class="btn btn-warning" href="{{ Auth::user()->downloadFile('PelayananProduktif', Session::get('year'))->file_path.Auth::user()->downloadFile('PelayananProduktif', Session::get('year'))->file_name }}" download="" ><i class="mdi mdi-note"></i>Download pdf file</a>
                            @endif
                            <a type="button" class="btn btn-warning" href="{{ route('PelayananProduktif.excel') }}" ><i class="mdi mdi-note"></i>Report</a>
                            <a type="button" class="btn btn-primary" href="{{ route('PelayananProduktif.add', ['year' => Session::get('year')]) }}"><i class="mdi mdi-plus"></i>Add</a>
                        </div>
                    </div>
                        ...
                        <table id="data" class="table table-bordered dt-responsive" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                            ...
                            <tbody>
                                ...
                                <tr style={{$key % 2 == 0?"background: gray":""}}>
                                    ...
                                    @if(isset($item->user) && $item->user->downloadFile('PelayananProduktif', Session::get('year')))
                                    <td><input type="checkbox" name="lock_upload" {{$item->user->downloadFile('PelayananProduktif', Session::get('year'))->status == 1 ? "checked":""}} class="data-lock-upload" id="{{$item->user->id}}"></td>
                                    @elseif(isset($item->user) && !$item->user->downloadFile('PelayananProduktif', Session::get('year')))
                                    <td><input type="checkbox" name="lock_upload" class="data-lock-upload" id="{{$item->user->id}}"></td>
                                    @else
                                    <td>-</td>
                                    @endif
                                    @if(isset($item->user) && $item->user->hasFile('PelayananProduktif', Session::get('year')))
                                    <td>
                                        @if($item->user->downloadFile('PelayananProduktif', Session::get('year'))->file_name != "-")
                                        <a type="button" class="btn btn-warning" href="{{ $item->user->downloadFile('PelayananProduktif', Session::get('year'))->file_path.$item->user->downloadFile('PelayananProduktif', Session::get('year'))->file_name }}" download="" ><i class="mdi mdi-note"></i>Download pdf file</a>
                                        @else
                                        -
                                        @endif
                                    </td>
                                ...
                                <tr style='{{$key % 2 == 0?"background: #e9e9e9":""}}'>
                                    ...
                                    <td><input type="number" {{$item->filterPelayananProduktif(Session::get('year'))->status == 1?"disabled":""}} name="jumlah_L" id="{{$item->filterPelayananProduktif(Session::get('year'))->id}}" value="{{$item->filterPelayananProduktif(Session::get('year'))->jumlah_L}}" class="form-control data-input" style="border: none"></td>
                                    <td><input type="number" {{$item->filterPelayananProduktif(Session::get('year'))->status == 1?"disabled":""}} name="jumlah_P" id="{{$item->filterPelayananProduktif(Session::get('year'))->id}}" value="{{$item->filterPelayananProduktif(Session::get('year'))->jumlah_P}}" class="form-control data-input" style="border: none"></td>
                                    ...
                                    <td><input type="number" {{$item->filterPelayananProduktif(Session::get('year'))->status == 1?"disabled":""}} name="standar_L" id="{{$item->filterPelayananProduktif(Session::get('year'))->id}}" value="{{$item->filterPelayananProduktif(Session::get('year'))->standar_L}}" class="form-control data-input" style="border: none"></td>
                                    <td id="standar_L{{$item->filterPelayananProduktif(Session::get('year'))->id}}">{{$item->filterPelayananProduktif(Session::get('year'))->jumlah_L > 0? number_format($item->filterPelayananProduktif(Session::get('year'))->standar_L/$item->filterPelayananProduktif(Session::get('year'))->jumlah_L * 100):0}}%</td>
                                    
                                    <td><input type="number" {{$item->filterPelayananProduktif(Session::get('year'))->status == 1?"disabled":""}} name="standar_P" id="{{$item->filterPelayananProduktif(Session::get('year'))->id}}" value="{{$item->filterPelayananProduktif(Session::get('year'))->standar_P}}" class="form-control data-input" style="border: none"></td>
                                    <td id="standar_P{{$item->filterPelayananProduktif(Session::get('year'))->id}}">{{$item->filterPelayananProduktif(Session::get('year'))->jumlah_P > 0? number_format($item->filterPelayananProduktif(Session::get('year'))->standar_P/$item->filterPelayananProduktif(Session::get('year'))->jumlah_P * 100):0}}%</td>
                                    ...
                                    <td><input type="number" {{$item->filterPelayananProduktif(Session::get('year'))->status == 1?"disabled":""}} name="risiko_L" id="{{$item->filterPelayananProduktif(Session::get('year'))->id}}" value="{{$item->filterPelayananProduktif(Session::get('year'))->risiko_L}}" class="form-control data-input" style="border: none"></td>
                                    <td id="risiko_L{{$item->filterPelayananProduktif(Session::get('year'))->id}}">{{$item->filterPelayananProduktif(Session::get('year'))->jumlah_L > 0? number_format($item->filterPelayananProduktif(Session::get('year'))->risiko_L/$item->filterPelayananProduktif(Session::get('year'))->jumlah_L * 100):0}}%</td>
                                    
                                    <td><input type="number" {{$item->filterPelayananProduktif(Session::get('year'))->status == 1?"disabled":""}} name="risiko_P" id="{{$item->filterPelayananProduktif(Session::get('year'))->id}}" value="{{$item->filterPelayananProduktif(Session::get('year'))->risiko_P}}" class="form-control data-input" style="border: none"></td>
                                    <td id="risiko_P{{$item->filterPelayananProduktif(Session::get('year'))->id}}">{{$item->filterPelayananProduktif(Session::get('year'))->jumlah_P > 0? number_format($item->filterPelayananProduktif(Session::get('year'))->risiko_P/$item->filterPelayananProduktif(Session::get('year'))->jumlah_P * 100):0}}%</td>
...
@push('scripts')
    <script>
        ...
        let risiko_L = $(this).parent().parent().find(`#risiko_L${id}`);
        let risiko_P = $(this).parent().parent().find(`#risiko_P${id}`);
        let risiko_LP = $(this).parent().parent().find(`#risiko_LP${id}`);
        let persen_risiko_LP = $(this).parent().parent().find(`#persen_risiko_LP${id}`);
		...
		$.ajax({
			'type' 	: 'POST',
			'url'	: '{{ route("pelayanan_produktif.store") }}',
			...
			success	: function(res){
                ...
                risiko_L.text(`${res.risiko_L}%`);
                risiko_P.text(`${res.risiko_P}%`);
                risiko_LP.text(`${res.risiko_LP}`);
                persen_risiko_LP.text(`${res.persen_risiko_LP}%`);
                ...
                $("#percentage_totalrisiko_LP").text(res.percentage_totalrisiko_LP)
                $("#totalstandar_LP").text(res.totalstandar_LP)
                $("#totalrisiko_LP").text(res.totalrisiko_LP)
                ...
                if(res.column == "jumlah_L"){
                    ...
                    $(`#percentage_risiko_L`).text(`${res.percent_risiko_L}%`);
                } else if(res.column == "jumlah_P"){
                    $(`#percentage_standar_P`).text(`${res.percent_standar_P}%`);
                    $(`#percentage_risiko_P`).text(`${res.percent_risiko_P}%`);
                }
		...
		$.ajax({
			...
			'data'	: {'id': id, 'mainFilter': 'filterPelayananProduktif'},
			success	: function(res){
                console.log(res);
                ...
                let newRow = `
                    <tr class="detail-row" style="background: #f9f9f9;">
                          <td></td>
                                ...
                                <td >${item.jumlah_P + item.jumlah_L > 0?((item.standar_P + item.standar_L)/(item.jumlah_P + item.jumlah_L)) * 100:0}%</td>
                                
                                <td >${item.risiko_L}</td>
                                <td >${item.jumlah_L > 0?(item.risiko_L/item.jumlah_L) * 100:0}%</td>
                                
                                <td >${item.risiko_P}</td>
                                <td >${item.jumlah_P > 0?(item.risiko_P/item.jumlah_P) * 100:0}%</td>
                                
                                <td >${item.risiko_P + item.risiko_L}</td>
                                <td >${item.jumlah_P + item.jumlah_L > 0?((item.risiko_P + item.risiko_L)/(item.jumlah_P + item.jumlah_L)) * 100:0}%</td>
        ...
        $("#filter").click(function(){
            let year = $("#tahun").val();
            window.location.href = "/pelayanan_produktif?year="+year;
    ...
    $('#data').on('click', '.data-lock', function(){
            ...
            $.ajax({
			'type' 	: 'POST',
			'url'	: '{{ route("PelayananProduktif.lock") }}',
    ...
    $('#datatable').on('click', '.btn-tambah-sasaran', function(){
        ...
        $('#data').on('click', '.data-lock-upload', function(){
			...
			'data'	: {'id': id, 'status': 1, 'year': year, 'name': "PelayananProduktif"},
			success	: function(res){
				console.log(res);
			}
		});
			...
			'type' 	: 'POST',
			'url'	: '{{ route("general.lock_upload") }}',
			'data'	: {'id': id, 'status': 0, 'year': year, 'name': "PelayananProduktif"},
			success	: function(res){
				console.log(res);
			}
		});
