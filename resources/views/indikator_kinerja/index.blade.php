                    ...
                    <div class="table-responsive">
                        ...
                        <div class="row justify-content-start mb-2">
                            <div class="col-md-10 d-flex justify-content-around gap-3">
                                @if(Auth::user()->downloadFile('IndikatorKinerja', Session::get('year')))
                                <form action="/upload/general" method="post" class="d-flex gap-5" enctype="multipart/form-data">
                                    @csrf
                                    <input type="hidden" name="name" value="IndikatorKinerja" id="">
                                    <input type="file" name="file_upload" id="" {{Auth::user()->downloadFile('IndikatorKinerja', Session::get('year'))->status == 1 ?"disabled":""}} class="form-control" placeholder="upload PDF file">
                                    <button type="submit" class="btn btn-success" {{Auth::user()->downloadFile('IndikatorKinerja', Session::get('year'))->status == 1?"disabled":""}}>Upload</button>
                                ...
                                @endif
                                @if(Auth::user()->hasFile('IndikatorKinerja', Session::get('year')) && Auth::user()->downloadFile('IndikatorKinerja', Session::get('year'))->file_name != "-")
                                    <a type="button" class="btn btn-warning" href="{{ Auth::user()->downloadFile('IndikatorKinerja', Session::get('year'))->file_path.Auth::user()->downloadFile('IndikatorKinerja', Session::get('year'))->file_name }}" download="" ><i class="mdi mdi-note"></i>Download pdf file</a>
                                @endif
                                <a type="button" class="btn btn-warning" href="{{url("/export_IndikatorKinerja")}}" ><i class="mdi mdi-note"></i>Report</a>
                            </div>
                        </div>
