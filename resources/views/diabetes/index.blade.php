                    ...
                    <div class="row justify-content-start mb-2">
                        <div class="col-md-10 d-flex justify-content-start gap-3">
                            ...
                            <form action="/upload/general" method="post" class="d-flex gap-5" enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="name" value="Diabetes" id="">
                                <input type="file" name="file_upload" id="" {{Auth::user()->downloadFile('Diabetes', Session::get('year'))->status == 1 ?"disabled":""}} class="form-control" placeholder="upload PDF file">
                                <button type="submit" class="btn btn-success" {{Auth::user()->downloadFile('Diabetes', Session::get('year'))->status == 1?"disabled":""}}>Upload</button>
                            ...
                            @endif
                            @if(Auth::user()->hasFile('Diabetes', Session::get('year')))
                                <a type="button" class="btn btn-warning" href="{{ Auth::user()->downloadFile('Diabetes', Session::get('year'))->file_path.Auth::user()->downloadFile('Diabetes', Session::get('year'))->file_name }}" download="" ><i class="mdi mdi-note"></i>Download pdf file</a>
                            @endif
                            <a type="button" class="btn btn-warning" href="{{ route('Diabetes.excel') }}" ><i class="mdi mdi-note"></i>Report</a>
                            <a type="button" class="btn btn-primary" href="{{ route('Diabetes.add', ['year' => Session::get('year')]) }}"><i class="mdi mdi-plus"></i>Add</a>
                        </div>
                    </div>
                        ...
                        <table id="data" class="table table-bordered dt-responsive" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                            ...
                            <tbody>
                                ...
                                <tr style={{$key % 2 == 0?"background: gray":""}}>
                                    ...
                                    @if(isset($item->user) && $item->user->downloadFile('Diabetes', Session::get('year')))
                                    <td><input type="checkbox" name="lock_upload" {{$item->user->downloadFile('Diabetes', Session::get('year'))->status == 1 ? "checked":""}} class="data-lock-upload" id="{{$item->user->id}}"></td>
                                    @elseif(isset($item->user) && !$item->user->downloadFile('Diabetes', Session::get('year')))
                                    <td><input type="checkbox" name="lock_upload" class="data-lock-upload" id="{{$item->user->id}}"></td>
                                    @else
                                    <td>-</td>
                                    @endif
                                    @if(isset($item->user) && $item->user->hasFile('Diabetes', Session::get('year')))
                                    <td>
                                        @if($item->user->downloadFile('Diabetes', Session::get('year'))->file_name != "-")
                                        <a type="button" class="btn btn-warning" href="{{ $item->user->downloadFile('Diabetes', Session::get('year'))->file_path.$item->user->downloadFile('Diabetes', Session::get('year'))->file_name }}" download="" ><i class="mdi mdi-note"></i>Download pdf file</a>
                                        @else
                                        -
                                        @endif
                                    </td>
                                ...
                                <tr style='{{$key % 2 == 0?"background: #e9e9e9":""}}'>
                                    ...
                                    <td><input type="number" disabled name="jumlah" id="{{$item->filterDiabetes(Session::get('year'))->id}}" value="{{$item->filterDiabetes(Session::get('year'))->jumlah}}" class="form-control data-input" style="border: none"></td>
                                    <td><input type="number" {{$item->filterDiabetes(Session::get('year'))->status == 1?"disabled":""}} name="pelayanan" id="{{$item->filterDiabetes(Session::get('year'))->id}}" value="{{$item->filterDiabetes(Session::get('year'))->pelayanan}}" class="form-control data-input" style="border: none"></td>
...
@push('scripts')
    <script>
		...
		$.ajax({
			...
			'data'	: {'id': id, 'mainFilter': 'filterDiabetes'},
			success	: function(res){
                console.log(res);
                ...
                let newRow = `
                    <tr class="detail-row" style="background: #f9f9f9;">
                     <td></td>
                                   <td></td>
                                    <td>${item.nama}</td>
                                    <td id="jumlah">${item.jumlah}</td>
                                    <td id="pelayanan">${item.pelayanan}</td>
                                    <td id="percentage">${item.jumlah>0?(item.pelayanan/item.jumlah) * 100:0}%</td>
                                    <td></td>
        ...
        $("#filter").click(function(){
            let year = $("#tahun").val();
            window.location.href = "/diabetes?year="+year;
    ...
    $('#datatable').on('click', '.btn-tambah-sasaran', function(){
        ...
        $('#data').on('click', '.data-lock-upload', function(){
			...
			'data'	: {'id': id, 'status': 1, 'year': year, 'name': "Diabetes"},
			success	: function(res){
				console.log(res);
			}
		});
			...
			'type' 	: 'POST',
			'url'	: '{{ route("general.lock_upload") }}',
			'data'	: {'id': id, 'status': 0, 'year': year, 'name': "Diabetes"},
			success	: function(res){
				console.log(res);
			}
		});
