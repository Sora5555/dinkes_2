    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0">Pagu Anggaran</h4>
...
$.ajax({
        ...
        success: (res) => {
            if(res.status == "success"){
                    ...
                    // merujuk_rpjmd(res.visi, res.misi, res.tujuan, res.sasaran, res.tujuanRenstra, res.sasaranRenstra, res.Iku, res.indikatorOpd, res.detailProgram, res.detailIndikatorProgram, res.detailKegiatan, res.detailIndikatorKegiatan, res.detailSubKegiatan, res.subKegiatan);
                        ...
                        if(elementdP.indikator_opd_id == elementIo.id){
                            ...
                            subKegiatan.forEach(elementSk => {
                                if(elementdSK.sub_kegiatan_id == elementSk.id){
                                    td += `
                                    <tr>
                            <td>Pagu Anggaran: Rp. ${parseInt(elementSk.pagu_indikatif).toLocaleString()}</td>
...
function merujuk_rpjmd(data){
    ...
    let valueOpd = $('#induk_opd').val();
            ...
            //                         td2 += `
            //                         <tr>
            //                 <td>Pagu Anggaraan: Rp. ${parseInt(elementSk.pagu_indikatif).toLocaleString()}</td>
            //                 <td><form action="/indikator_opd2/${elementSk.id}" method="POST" class="delete-data">
            //                     @csrf
            //                     <div class="btn-group" role="group" aria-label="Action Button">
            //                         <input type="hidden" name="_method" value="DELETE">
